# firewall-sensitive-word

## Purpose

`quickboot-common` 提供的 **安全防火墙 — 敏感词过滤**：在 Servlet 链**尽早**对 **Query/Form** 与 **`application/json`** 请求体做检测或替换；配置前缀 `qc.security.firewall.sensitive-word`（`enabled`、`ignoreUrls`、黑白名单资源、`REPLACE`/`THROW`）；`THROW` 时业务码 **30501** 与统一 **R** JSON；依赖 **houbb `sensitive-word`**、Spring **`ResourceLoader`**、**Jackson**。
