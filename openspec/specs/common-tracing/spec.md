...
约定 `quickboot-common` 与 Web 侧如何通过 **SLF4J MDC**（键 `traceId` / `spanId`）与 **Micrometer Tracing** 协作：公开读取入口为 `TraceIds`；`R` 在构建时写入 `traceId`；span 的创建与 W3C 上下文传播由 Spring Boot / Micrometer 负责，不在 common 内手工创建 span。运维通过 `management.tracing`、`management.otlp.tracing` 配置采样与 OTLP 导出。
