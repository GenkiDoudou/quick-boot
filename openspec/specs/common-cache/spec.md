...
`quickboot-common` 提供的 Spring Cache 能力：按 `spring.cache.type` 在本地 Caffeine 与 Redis 之间切换；支持注解 `cacheNames` 的 `cacheName#ttlSeconds` 动态 TTL；Redis 侧统一字符串 key 与 JSON value，并具备事务感知语义。
