...
`quickboot-common` 提供的 **安全防火墙 — CORS 跨域**：以 Servlet **Filter**（或 Spring 等价机制）在命中路径上输出 CORS 响应头、处理预检 OPTIONS；配置前缀 **`qc.security.firewall.cors`**（`enabled`、`allowedOrigins`、`allowedMethods` 等）；**`enabled` 默认关闭**；与后续拦截型防火墙 Filter 协同时，跨域场景下**最终响应仍应带 CORS 头**。
