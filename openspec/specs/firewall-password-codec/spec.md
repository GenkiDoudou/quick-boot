# firewall-password-codec

## Purpose

`quickboot-common` 提供的 **安全防火墙 — 密码/认证串编解码**：统一 `{codecId}payload` 前缀，支持 **bcrypt**（Hutool，不可逆）与 **SM4**（Hutool 国密可逆，负载十六进制）；多 **keyId** 经 `PasswordCodec#setProperties` 在初始化阶段注入；配置前缀 `qc.security.firewall.password.codec` 与 Spring Boot 自动配置集成。
