...
`quickboot-common` 提供的 **`I18nUtil`**：在 Spring 容器内通过 **`MessageSource`** 与 **`LocaleContextHolder`** 按错误码与占位参数解析国际化文案；不负责词条内容维护与完整 Web 语言切换链路。

## Requirements

### Requirement: I18nUtil 提供基于 MessageSource 的 getMessage

系统 MUST 在 `quickboot-common` 提供 **`I18nUtil`**，通过 Spring **`MessageSource`** 解析消息键 **`code`**。必须至少支持 **`getMessage(String code)`**、**`getMessage(String code, Object[] args)`**、**`getMessage(String code, Object[] args, String defaultMessage)`**。在未显式传入 **`Locale`** 的重载中，系统 MUST 使用 **`LocaleContextHolder.getLocale()`** 作为解析 **`Locale`**。不提供单独的 **`Object...`** 形式 **`args`** 重载（与 **`Object[]`** 擦除后方法签名冲突，调用方应传入数组）。
