# firewall-method-and-host

## Purpose

`quickboot-common` 提供的 **安全防火墙 — HTTP Method 与 Host 白名单**：在 Servlet 链中对 **`allowedMethods`**、**`allowedHosts`** 做校验，支持 **`excludeUrls`** 排除；配置前缀 **`qc.security.firewall.method-and-host`**；**`enabled` 默认关闭**；拦截时 **HTTP 200** + 统一 **`R` JSON**，业务码 **`30401`**（method）、**`30402`**（host），并与 **`ServletUtils`** 的 i18n + 兜底文案能力配合。
