...
/**
 * 修复 JimuBI {@code DragRedisUtil} 在无 Redis 时本地缓存 {@code increment} 的缺陷：
 * 对 {@link org.springframework.cache.Cache.ValueWrapper} 误用 {@code toString()} 导致
 * {@code NumberFormatException: ValueWrapper for [1]}。
 */
final class FixedDragRedisUtil extends org.jeecg.modules.drag.util.f {

    private static final Field REDIS_TEMPLATE_FIELD;

    static {
        try {
            REDIS_TEMPLATE_FIELD = org.jeecg.modules.drag.util.f.class.getDeclaredField("redisTemplate");
