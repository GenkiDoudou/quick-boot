...
/**
 * AI 模型 API Key 的 SM4 加解密与脱敏，模式对齐 {@link io.github.genkidoudou.web.knowledge.mcp.support.McpSecretSupport}。
 */
public final class AiSecretSupport {

    /** 列表与默认详情中的脱敏占位符。 */
    public static final String MASK = "******";

    private AiSecretSupport() {
    }
    ...
    public static String resolvePlainValue(PasswordCodec codec, String valueType, String stored) {
        if (StrUtil.isBlank(valueType)) {
            valueType = AiApiKeyType.PLAIN;
        }
        return switch (valueType) {
            case AiApiKeyType.SECRET -> resolvePlainSecret(codec, stored);
            case AiApiKeyType.ENV_REF -> resolveEnvRef(stored);
    ...
    public static boolean isKeepExistingSecret(String valueType, String submitted) {
        return AiApiKeyType.SECRET.equals(valueType) && StrUtil.isBlank(submitted);
    }
