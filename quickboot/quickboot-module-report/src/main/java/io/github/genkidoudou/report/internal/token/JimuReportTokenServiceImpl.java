...
import io.github.genkidoudou.report.internal.security.JimuShareUriMatcher;
...
public class JimuReportTokenServiceImpl implements JmReportTokenServiceI {
    ...
    private static final String[] SHARE_READONLY_PERMISSIONS = new String[]{
            ...
            // /drag/view 预览拉数会校验该权限；仅 getTotalData 不够
            "drag:analysis:sql"
    };

    /** 积木 BI 设计器敏感操作权限（管理员 *:*:* 或 role_key=admin 时合并） */
    private static final String[] JIMU_BI_ADMIN_PERMISSIONS = new String[]{
            ...
            // 报表设计器会探测 AI 表元数据接口；本期不迁 AI Key，仅放行权限校验
            "jmreport:ai:tableMeta"
    };
    ...
    public String[] getPermissions(String token) {
        ...
        List<String> roles = jimuAuthBridge.listRoleKeysByToken(token);
        if (isJimuBiAdmin(perms, roles)) {
    ...
    /**
     * 现网管理员未必有 {@code *:*:*}，故同时认 role_key=admin。
     */
    private static boolean isJimuBiAdmin(List<String> perms, List<String> roles) {
        if (perms != null && perms.stream().anyMatch(p -> "*:*:*".equals(p) || "*".equals(p))) {
            return true;
        }
