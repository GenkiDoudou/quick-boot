...
public class WorkflowEngine {
    ...
    public Map<String, Object> execute(Long runId, String graphJson, WorkflowContext context) {
        ...
        try {
            while (!queue.isEmpty()) {
                ...
                if (executed.contains(nodeId) || skipped.contains(nodeId)) {
                    continue;
                }
                if (!canExecute(nodeId, edges, executed, skipped)) {
                    queue.add(nodeId);
                    if (queue.size() > nodeMap.size() * 2) {
                        break;
                    }
                    continue;
                }
                ...
                if (result.isSuccess()) {
                    ...
                    saveStep(runId, node, "SUCCESS", result.getOutputs(), null, stepDuration, orderNo);
                    streamEmitter.emitStepEnd(runId, nodeId, "SUCCESS", stepDuration,
                        traceSanitizer.sanitizeMap(result.getOutputs()));
                    executed.add(nodeId);
                    ...
                    enqueueSuccessors(node, nodeId, edges, result.getBranchHandle(), queue, skipped, nodeMap.keySet());
                } else {
                    errorMsg = result.getErrorMessage();
                    saveStep(runId, node, "FAILED", Map.of(), errorMsg, stepDuration, orderNo);
                    streamEmitter.emitStepEnd(runId, nodeId, "FAILED", stepDuration, Map.of());
    ...
    private void enqueueSuccessors(GraphNodeDto node, String nodeId, List<GraphEdgeDto> edges,
                                   String branchHandle, Queue<String> queue, Set<String> skipped,
        ...
        for (GraphEdgeDto edge : edges) {
            ...
            if (isBranchNode) {
                ...
                if (branchHandle != null && handle != null && !handle.equals(branchHandle)) {
                    markSkippedSubtree(edge.getTarget(), edges, skipped, allNodeIds);
                    continue;
                }
            }
            if (!skipped.contains(edge.getTarget())) {
    ...
    private boolean canExecute(String nodeId, List<GraphEdgeDto> edges, Set<String> executed, Set<String> skipped) {
    ...
    private void saveStep(Long runId, GraphNodeDto node, String status, Map<String, Object> outputs,
                          String errorMsg, long durationMs, int orderNo) {
        WfRunStep step = new WfRunStep();
        step.setRunId(runId);
        step.setNodeId(node.getId());
        step.setNodeType(node.getType());
        step.setStatus(status);
        step.setOutputsJson(JSONUtil.toJsonStr(traceSanitizer.sanitizeMap(outputs)));
