...
/**
 * 工作流运行调试 composable（Debug / 异步 SSE、节点 runStatus + 画布节点 Trace）。
...
export function useWorkflowRun({ workflowId, nodes, focusNode, getGraph }) {
  ...
  /**
   * 将单步 Trace 写入对应画布节点（Coze 风格：I/O 挂在节点上）。
   * @param {string} nodeId
   * @param {object} step
   * @param {{ expand?: boolean }} [opts]
   */
  function applyStepToNode(nodeId, step, opts = {}) {
    if (!nodeId) return
    nodes.value = nodes.value.map((n) => {
      if (n.id !== nodeId) return n
      const prevTraces = Array.isArray(n.data?.runTraces) ? [...n.data.runTraces] : []
      const orderNo = step.orderNo
      const idx = prevTraces.findIndex((t) => t.orderNo === orderNo)
      if (idx >= 0) prevTraces[idx] = step
      else prevTraces.push(step)
      prevTraces.sort((a, b) => (a.orderNo ?? 0) - (b.orderNo ?? 0))

      const status =
        step.status === 'FAILED' ? 'FAILED' : step.status === 'RUNNING' ? 'RUNNING' : 'SUCCESS'
      ...
      return {
        ...
        data: {
          ...
          runStatus: status,
          runTrace: step,
          runTraces: prevTraces,
          runTraceExpanded: opts.expand ?? n.data?.runTraceExpanded ?? step.status === 'RUNNING'
        }
      }
    })
  }
  ...
  function toggleNodeTraceExpanded(nodeId) {
    nodes.value = nodes.value.map((n) => {
      if (n.id !== nodeId) return n
      return {
        ...n,
        data: {
          ...n.data,
          runTraceExpanded: !n.data?.runTraceExpanded
        }
      }
    })
  }
  ...
  function syncAllStepsToNodes(steps, { expand = true } = {}) {
    ;(steps || []).forEach((step) => {
      applyStepToNode(step.nodeId, step, { expand })
    })
  }
    ...
    unsubscribeStream = subscribeRunStream(runId, {
      onEvent: (event, data) => {
        if (event === 'step_start') {
          ...
          applyStepToNode(data.nodeId, step, { expand: true })
  ...
  async function runTest(inputs, useStream) {
    ...
    try {
      if (stream) {
        ...
        syncAllStepsToNodes(traceSteps.value, { expand: true })
