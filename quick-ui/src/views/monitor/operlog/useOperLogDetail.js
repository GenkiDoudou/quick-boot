...
import { getOperlog, listOperlog } from '@/api/monitor/operlog'

/**
 * 按 traceId 打开操作日志详情弹窗。
 * @returns {{
 ...
 *   openOperLogByOperId: (operId: number) => Promise<void>,
 * }}
 */
export function useOperLogDetail() {
  ...
  async function openOperLogByTraceId(traceId) {
    ...
    try {
      const res = await listOperlog({ traceId: normalized, pageNum: 1, pageSize: 1 })
