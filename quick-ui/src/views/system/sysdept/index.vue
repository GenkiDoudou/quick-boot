...
const handleDelete = (row) => {
  ...
  if (!row || !row.deptId) {
    proxy.$modal.msgError("删除失败：无效的部门数据");
    return;
  }
...
const data = reactive({
  ...
  rules: {
    ...
    phone: [ {validator: proxy.$validate.validatePhone, trigger: "blur"}],
    email: [ {validator: proxy.$validate.validateEmail, trigger: "blur"}]
  },
});
