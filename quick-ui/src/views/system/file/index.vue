<template>
  <div class="app-container">
    ...
    <c7-dialog v-model="previewVisible" title="预览" :footer="false" width="720px" @closed="handlePreviewClosed">
      <div v-loading="previewLoading" class="preview-body">
        ...
        <iframe
          ...
          title="PDF 预览"
        />
        <el-empty v-else-if="!previewLoading" :description="previewError || '暂无可预览内容'" />
      </div>
    </c7-dialog>
  </div>
</template>
...
import { downloadFile, fetchFilePreviewBlob, listFile, removeFile, uploadFile } from '@/api/system/file'
...
// 预览：鉴权 Blob + objectURL，避免裸链无 Token 触发 ClientBasic 600
...
function resolvePreviewBlob(blob, ext) {
  if (!(blob instanceof Blob)) {
    return blob
  }
  const type = String(blob.type || '')
  if (type && !type.includes('application/json') && !type.includes('text/plain')) {
    return blob
  }
  if (isPdfExt(ext)) {
  ...
  if (isVideoExt(ext)) {
    return new Blob([blob], { type: `video/${ext}` })
  }
  return blob
}
...
async function handlePreview(row) {
  if (row?.fileId == null) {
    ElMessage.error('文件 ID 为空')
    return
  }
  if (!isImageExt(row.ext) && !isVideoExt(row.ext) && !isPdfExt(row.ext)) {
    ElMessage.info('该类型不支持在线预览，已改为下载')
    return handleDownload(row)
  }
  revokePreviewUrl()
  previewError.value = ''
  previewLoading.value = true
  previewType.value = isImageExt(row.ext) ? 'image' : isVideoExt(row.ext) ? 'video' : 'pdf'
  previewVisible.value = true
  try {
    const raw = await fetchFilePreviewBlob(row.fileId)
    const blob = resolvePreviewBlob(raw, row.ext)
    previewUrl.value = URL.createObjectURL(blob)
  } catch (e) {
    previewError.value = e?.message || '预览失败'
    // 拦截器已提示时不再重复；弹窗保留以便看到空态
  } finally {
